...
const ChatPage = (props) => {
  ...
  useEffect(() => {
    const fetchChatHistory = async () => {
      try {
        const res = await fetch(`/api/chat-history/${profile.username}`);
        const data = await res.json();
        const formatted = (data.context || []).map((msg, idx) => ({
          id: idx,
          message: msg.text,
          type: msg.type, // "bot" or "user"
        }));
    ...
    if (profile?.username) fetchChatHistory();
  }, [profile]);

  const handleGoBack = async () => {
    try {
      // Push chat history to backend
      await fetch(`/api/store-history/${profile.username}`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ context: chatHistory }),
      });
